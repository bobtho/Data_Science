...
Parkinson.reindex(np.random.permutation(Parkinson.index))
...
Best_features = list(zip(X, rfe.support_))
...
X_new = Parkinson[new_features]
print(X_new) 
           
# Compute mean and Std deviation of training set
...
#  Dont need
#Xn_train, Xn_test, yn_train, yn_test = train_test_split(X_new, y, test_size = 0.20, random_state = 123, stratify=y)
#
...
Parkinson_Xtrain_Un_index=[i for i in X_train.index if i in Parkison_Unacc_index]
len(Parkinson_Xtrain_Un_index)       #   67 (i in PIMA_NonDiabetic_index)
print(Parkinson_Xtrain_Un_index)

# The number of Minority Class in X_train 

Parkinson_Xtrain_Acc_index=[i for i in X_train.index if i not in Parkison_Unacc_index]
len(Parkinson_Xtrain_Acc_index)       # 33 (i in CC_Fraud_index)

# Downsampling Majority Class 
Parkinson_Downsample = resample(Parkinson_Xtrain_Un_index, replace = False, n_samples = 33, random_state = 123)
len(Parkinson_Downsample)

# Randomly Sample the Majority Class wrt to Minority Class

Rand_index = np.random.choice(Parkinson_Xtrain_Un_index, Parkinson_Acc,replace='False')
len(Rand_index)

# Concatenate the Minority Indexes and Majority Indexes
Parkinson_Undersample_index =  np.concatenate([Parkinson_Xtrain_Acc_index,Parkinson_Downsample])
...
Parkinson_Undersample = X_new.iloc[Parkinson_Undersample_index]
