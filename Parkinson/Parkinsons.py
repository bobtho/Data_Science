...
# Shuffle the Datafrome 
Parkinson = Parkinson.reindex(np.random.permutation(Parkinson.index))
print(Parkinson)
...
y = Parkinson['Class']
X = Parkinson.drop(['Class'], axis = 1)    # Axis = 1 refers to Column


print (X)            # [126 rows x 310 columns]
print (y)            # 126
...
print('Labels counts in y:', np.bincount(y))                        #  [42 84] 
...
Parkinson_X_train_Un_index=[i for i in X_train.index if i in Parkison_Unacc_index]
len(Parkinson_X_train_Un_index)       #   67 (i in PIMA_NonDiabetic_index)
print(Parkinson_X_train_Un_index)

# The number of Minority Class in X_train 

Parkinson_X_train_Acc_index=[i for i in X_train.index if i not in Parkison_Unacc_index]
len(Parkinson_X_train_Acc_index)       # 33 (i in CC_Fraud_index)

# Downsampling Majority Class 
Parkinson_Downsample = resample(Parkinson_X_train_Un_index, replace = False, n_samples = 33, random_state = 123)
len(Parkinson_Downsample)

# Randomly Sample the Majority Class wrt to Minority Class

Rand_index = np.random.choice(Parkinson_X_train_Un_index, Parkinson_Acc,replace='False')
len(Rand_index)

# Concatenate the Minority Indexes and Majority Indexes
Parkinson_Undersample_index =  np.concatenate([Parkinson_X_train_Acc_index,Parkinson_Downsample])
...
Parkinson_Undersample = Parkinson.iloc[Parkinson_Undersample_index]
...
print(x_under_train.shape, x_under_test.shape)      # (52, 310) (14, 310)
...
SVM_recall =  recall_score(SVM_Pred, y_under_test, average = 'weighted')
...
print(classification_report(y_under_test, SVM_Pred, sample_weight=precision))
...
Parkinson_Upsample = resample(Parkinson_X_train_Acc_index, replace = True, n_samples = 67, random_state = 123)
len(Parkinson_Upsample)

# Concatenate the Minority Indexes and Majority Indexes
Parkinson_Upsample_index =  np.concatenate([Parkinson_X_train_Acc_index,Parkinson_Upsample])
